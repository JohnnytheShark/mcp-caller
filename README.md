...
> **A lightweight CLI tool (*curl for MCP*)** to discover, inspect, and invoke Model Context Protocol (MCP) server tools directly from your terminal — with an interactive schema-prompting wizard or headless automation flags.
...
- **⚡ Zero Runtime Overhead**: Single compiled native binary built in Rust (<10MB), instant startup, zero 
ode_modules or Python runtime dependencies needed for the caller.
- **🪄 Dynamic Schema Form Engine**:
  ...
  - Validates numeric inputs (integer, 
umber) and parses JSON arrays/objects.
  - Automatically uses schema default values and displays field descriptions.
- **🔍 Auto-Discovery**: Automatically loads MCP servers configured in:
...
- **🧹 Clean Code Architecture**: Decoupled transports ([Transport](src/mcp/transport.rs) trait), S.P.I.K.E.R. audited, zero unneeded dependencies, with 20 unit tests.
...
cargo build --release
`
The compiled binary will be located at 	arget/release/mcp-caller (or mcp-caller.exe on Windows).
...
- 🎓 **[Tutorials](docs/tutorials/index.md)**: [Getting Started — Your First MCP Tool Call](docs/tutorials/getting-started.md)
- 🛠️ **[How-to Guides](docs/how-to/index.md)**:
